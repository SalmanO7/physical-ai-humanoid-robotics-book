...
Represents user input in natural language used to direct the robot.

-   **Attributes**:
    -   `text_transcript`: The raw text converted from the user's voice input.
    -   `semantic_meaning`: The interpreted high-level intent and parameters extracted from the text (e.g., "navigate to kitchen", "pick up red cube").
...
Represents a sequence of high-level tasks generated by the LLM to achieve a given `Voice Command`.
